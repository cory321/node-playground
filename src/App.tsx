...
function App() {
	...
	const [connectingFromPort, setConnectingFromPort] = useState<string | null>(
		null,
	); // For multi-port output nodes
	...
	// Comparison context
	const { locations: comparisonLocations, togglePanel: toggleComparisonPanel } =
		useComparison();

	// Image library context
	const { images: libraryImages, togglePanel: toggleImageLibrary } =
		useImageLibrary();
	...
	const addImageSourceNode = useCallback(() => {
		const node = createNode(
			'image-source',
			(window.innerWidth / 2 - transform.x - 170) / transform.scale,
			(window.innerHeight / 2 - transform.y - 190) / transform.scale,
		);
		if (node) setNodes((prev) => [...prev, node]);
	}, [transform]);
	...
	const addComparisonDataNode = useCallback(() => {
		const node = createNode(
			'comparison-data',
			(window.innerWidth / 2 - transform.x - 240) / transform.scale,
			(window.innerHeight / 2 - transform.y - 290) / transform.scale,
		);
		if (node) setNodes((prev) => [...prev, node]);
	}, [transform]);

	const addSEOOptimizationNode = useCallback(() => {
		const node = createNode(
			'seo-optimization',
			(window.innerWidth / 2 - transform.x - 250) / transform.scale,
			(window.innerHeight / 2 - transform.y - 320) / transform.scale,
		);
		if (node) setNodes((prev) => [...prev, node]);
	}, [transform]);

	const addDesignPromptNode = useCallback(() => {
		const node = createNode(
			'design-prompt',
			(window.innerWidth / 2 - transform.x - 210) / transform.scale,
			(window.innerHeight / 2 - transform.y - 240) / transform.scale,
	...
	const addBrandDesignNode = useCallback(() => {
		const node = createNode(
			'brand-design',
			(window.innerWidth / 2 - transform.x - 200) / transform.scale,
			(window.innerHeight / 2 - transform.y - 275) / transform.scale,
		);
		if (node) setNodes((prev) => [...prev, node]);
	}, [transform]);

	const addDataViewerNode = useCallback(() => {
		const node = createNode(
			'data-viewer',
			(window.innerWidth / 2 - transform.x - 210) / transform.scale,
			(window.innerHeight / 2 - transform.y - 240) / transform.scale,
	...
	const getPortConnections = useCallback(
		(nodeId: string, portId: string) => {
			return connections.filter(
				(c) => c.fromId === nodeId && c.fromPort === portId,
			);
		},
		[connections],
	);
	...
	return (
      ...
      `}</style>
			...
			<Toolbar
				...
				onAddImageSourceNode={addImageSourceNode}
				...
				onAddComparisonDataNode={addComparisonDataNode}
				onAddSEOOptimizationNode={addSEOOptimizationNode}
			...
			>
				...
				<NodeRenderer
					...
					onInputPortMouseUp={
						connectingFromPort
							? completeConnectionToInputFromPort
							: completeConnectionToInput
					}
					...
					getIncomingComparisonDataData={getIncomingComparisonDataData}
					getIncomingSEOOptimizationData={getIncomingSEOOptimizationData}
