...
interface ImageLibraryActions {
  addImage: (dataUrl: string, prompt: string, aspectRatio: string, nodeId?: string) => Promise<GeneratedImage | null>;
...
export function ImageLibraryProvider({ children }: ImageLibraryProviderProps) {
  ...
  const addImage = useCallback(async (
    ...
    try {
      const newImage = await uploadGeneratedImage(dataUrl, prompt, aspectRatio, nodeId);
  ...
  // Remove an image
  const removeImage = useCallback(async (id: string, storagePath: string): Promise<boolean> => {
