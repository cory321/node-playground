...
interface UseProviderDiscoveryReturn {
  runDiscovery: (category: string, city: string, state: string | null) => Promise<void>;
  stopDiscovery: () => void;
...
export function useProviderDiscovery({
  ...
  const runDiscovery = useCallback(
    async (category: string, city: string, state: string | null) => {
      ...
      try {
        // Fetch providers from SerpAPI
        const result = await discoverProviders(category, city, state);
    ...
    [isDiscovering, hasApiKey, nodeId, updateNode, resetProgress, updateProgress]
  );
