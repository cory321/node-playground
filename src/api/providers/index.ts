 ...
 * Uses SerpAPI Google Maps engine to find and score providers
 * @param limit - Number of providers to request (default 10, max ~20 from Google Maps)
 */
	...
	try {
		...
		const response = await fetch(`${supabaseUrl}/functions/v1/serp-proxy`, {
			...
			body: JSON.stringify({
				...
				city,
				state,
				includeLocalPack: limit <= 3, // Use Local Pack only for 3 or fewer
				limit,
			}),
		});
		...
		if (!response.ok) {
			...
			throw new Error(
				errorData.error ||
					`SERP proxy error: ${response.status} ${response.statusText}`,
			);
		}
