...
/**
 * Call Claude API with vision for code generation
 * Optimized for generating code while viewing a screenshot reference
 * Uses Claude Opus for highest quality code output
 */
export async function callClaudeWithVisionForCode(
  prompt: string,
  imageUrl: string,
  ...
  // Build multimodal message content with image first for visual context
  const messageContent: ClaudeMessageContent[] = [
    {
      type: 'image',
      source: imageSource,
  ...
  if (!response.ok) {
    const error: ClaudeResponse = await response.json().catch(() => ({}));
    throw new Error(error.error?.message || `Claude Vision Code API error: ${response.status}`);
  }
