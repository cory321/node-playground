...
export function ProviderDiscoveryNode({
	...
	const { runDiscovery, stopDiscovery, isDiscovering, hasApiKey } =
		useProviderDiscovery({
			nodeId: node.id,
			updateNode,
			providerLimit: node.providerLimit || 10,
		});

	// Handle provider limit change
	const handleLimitChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
		updateNode(node.id, { providerLimit: parseInt(e.target.value, 10) });
	};

	// Determine the category to use
	const effectiveCategory =
		incomingData?.category || node.manualCategory || TIER1_CATEGORIES[0];
	...
	const handleToggleSelected = useCallback(
		(providerId: string) => {
			const currentSelected = node.selectedProviderIds || [];
			const isCurrentlySelected = currentSelected.includes(providerId);
			const newSelected = isCurrentlySelected
				? currentSelected.filter((id) => id !== providerId)
				: [...currentSelected, providerId];
			updateNode(node.id, { selectedProviderIds: newSelected });
		},
		[node.id, node.selectedProviderIds, updateNode],
	);

	// Select all providers
	const handleSelectAll = useCallback(() => {
		const allIds = node.providers.map((p) => p.id);
		updateNode(node.id, { selectedProviderIds: allIds });
	}, [node.id, node.providers, updateNode]);

	// Deselect all providers
	const handleDeselectAll = useCallback(() => {
		updateNode(node.id, { selectedProviderIds: [] });
	}, [node.id, updateNode]);
		...
		>
			...
			{!incomingData?.city ? (
				...
				<>
					...
					{/* Provider Limit Selector */}
					<div className="flex items-center gap-3 px-3 py-2 bg-slate-800/50 border border-slate-700/50 rounded-lg">
						<label className="text-[10px] uppercase tracking-wider text-slate-500 font-mono whitespace-nowrap">
							Max Providers
						</label>
						<select
							value={node.providerLimit || 10}
							onChange={handleLimitChange}
							disabled={isDiscovering}
							className="flex-1 px-2 py-1 bg-slate-700/50 border border-slate-600/50 rounded text-xs text-slate-200 focus:outline-none focus:border-teal-500/50"
						>
							<option value={3}>3 (Local Pack)</option>
						...
						<span className="text-[9px] text-slate-500">
							{(node.providerLimit || 10) > 3 ? 'Google Maps' : 'Local Pack'}
						</span>
					</div>
			...
			{hasResults && counts && (
				<div className="flex items-center justify-between px-3 py-2 bg-slate-800/50 border border-slate-700/50 rounded-lg">
					<div className="flex items-center gap-3 text-[10px] font-mono">
						<span className="text-slate-400">{counts.total} providers</span>
						{counts.p1 > 0 && (
					...
					<div className="flex items-center gap-3">
						{(node.selectedProviderIds?.length || 0) > 0 && (
							<span className="text-[10px] text-cyan-400 font-mono">
								{node.selectedProviderIds.length} exporting
							</span>
						)}
						{counts.contacted > 0 && (
			...
			{hasResults && (
				<ProviderTable
					...
					selectedIds={node.selectedProviderIds || []}
