	...
	const handleGenerate = useCallback(async () => {
		...
		if (!effectivePrompt?.trim()) {
			updateNode(node.id, {
				status: 'error',
				error:
					'No prompt provided. Enter a prompt or connect an upstream node.',
			});
			return;
		}
		...
		>
				...
				<div className="flex-1 bg-slate-950/60 border border-slate-700/50 rounded-lg px-3 py-2 text-xs text-slate-400">
					...
					{customRatioMismatch && (
						<span
							...
							title={`Your ratio ${getDisplayAspectRatio()} mapped to closest: ${getApiAspectRatio()}`}
						>
							≈
						</span>
					)}
			...
			{showSettings && (
				<div className="flex flex-col gap-2 p-3 bg-slate-950/60 border border-slate-700/50 rounded-lg">
					{/* Mode Toggle */}
					<div className="flex items-center gap-2">
						<span className="text-[10px] uppercase tracking-wider text-slate-500 w-12">
							Mode
						</span>
						<div className="flex flex-1 bg-slate-800 rounded-lg p-0.5">
							<button
								onClick={() =>
									updateNode(node.id, { aspectRatioMode: 'preset' })
								}
							...
							<button
								onClick={() =>
									updateNode(node.id, { aspectRatioMode: 'custom' })
								}
								className={`flex-1 px-2 py-1 text-[10px] uppercase tracking-wider rounded-md transition-all ${
									node.aspectRatioMode === 'custom'
										? 'bg-cyan-600 text-white'
					...
					{node.aspectRatioMode === 'custom' && (
						<div className="flex items-center gap-2">
							...
							<input
								...
								onChange={(e) =>
									updateNode(node.id, {
										customHeight: parseInt(e.target.value) || null,
									})
								}
								placeholder="Height"
								className="flex-1 bg-slate-800 border border-slate-700/50 rounded-lg px-2 py-1.5 text-xs text-slate-200 focus:outline-none focus:ring-1 focus:ring-cyan-500/30 w-20"
							...
							<div className="text-[10px] pl-14 space-y-0.5">
								...
								{customRatioMismatch && (
									<div className="text-amber-400/80">
										→ Using closest:{' '}
										<span className="font-mono">{getApiAspectRatio()}</span>
									</div>
								)}
							</div>
						)}
