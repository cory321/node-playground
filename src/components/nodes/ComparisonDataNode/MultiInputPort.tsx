...
// Input port configuration for Comparison Data Node
...
// Default ports for Comparison Data node
// Only blueprint is needed - enriched providers and local knowledge are passed through from SitePlannerOutput
export const COMPARISON_DATA_INPUT_PORTS: InputPortConfig[] = [
	{ id: 'blueprint', label: 'Blueprint', icon: FileText, yOffset: 100, required: true },
];

// Port spacing constant for connection layer calculations
export const COMPARISON_DATA_INPUT_BASE_OFFSET = 100;
export const COMPARISON_DATA_INPUT_SPACING = 44;
...
// Port colors for multi-input (using red theme for Comparison Data)
const PORT_COLORS = {
	default: { border: 'border-slate-600', bg: 'bg-slate-600' },
	connected: { border: 'border-red-500', bg: 'bg-red-500' },
	ready: { border: 'border-green-500', bg: 'bg-green-500' },
	hovered: { border: 'border-red-400', bg: 'bg-red-400' },
	active: { border: 'border-red-400', bg: 'bg-red-400' },
	...
	return (
		<>
			{ports.map((port) => {
                ...
                ${colors.border}
                ${isHovered ? 'scale-150 shadow-[0_0_15px_rgba(239,68,68,0.6)]' : ''}
