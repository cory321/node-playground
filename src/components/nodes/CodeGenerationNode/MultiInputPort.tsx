...
// Input port configuration for Code Generation Node
...
// Input ports for Code Generation node (4 inputs from upstream nodes)
// Note: Editorial and Comparison data are passed through via SEO Package's sourceData
export const CODE_GEN_INPUT_PORTS: InputPortConfig[] = [
	{
		id: 'sitePlan',
		label: 'Site Plan',
		icon: FileText,
...
export const CODE_GEN_INPUT_BASE_OFFSET = 100;
export const CODE_GEN_INPUT_SPACING = 44;
...
// Port colors for multi-input (using emerald theme for Code Generation)
const PORT_COLORS = {
	default: { border: 'border-slate-600', bg: 'bg-slate-600' },
	connected: { border: 'border-emerald-500', bg: 'bg-emerald-500' },
	ready: { border: 'border-green-500', bg: 'bg-green-500' },
	hovered: { border: 'border-emerald-400', bg: 'bg-emerald-400' },
	active: { border: 'border-emerald-400', bg: 'bg-emerald-400' },
	...
	return (
		<>
			{ports.map((port) => {
                ...
                ${colors.border}
                ${isHovered ? 'scale-150 shadow-[0_0_15px_rgba(5,150,105,0.6)]' : ''}
