...
// Comparison Data Node uses same spacing values
const COMPARISON_DATA_INPUT_BASE_OFFSET = 100;
const COMPARISON_DATA_INPUT_SPACING = 44;
...
export function ConnectionLayer({
	...
	// Comparison Data input port order for index lookup
	const COMPARISON_DATA_PORT_ORDER = [
		'blueprint',
		'enriched-providers',
		'local-knowledge',
	];
	...
	const getPortPosition = (
		...
		if (type === 'out' && portId && isCategorySelectorNode(node)) {
			...
			if (portIndex !== -1) {
				...
				const yOffset =
					MULTI_PORT_BASE_OFFSET +
					portIndex * MULTI_PORT_SPACING +
					MULTI_PORT_SPACING / 2;
				return {
					x: node.x + node.width,
					y: node.y + yOffset,
				};
		...
		if (type === 'in' && portId && isSitePlannerNode(node)) {
			...
			if (portIndex !== -1) {
				...
				const yOffset =
					SITE_PLANNER_INPUT_BASE_OFFSET +
					portIndex * SITE_PLANNER_INPUT_SPACING;
				return {
					x: node.x,
					y: node.y + yOffset,
				};
		...
		if (type === 'in' && portId && isProviderProfileGeneratorNode(node)) {
			...
			if (portIndex !== -1) {
				...
				const yOffset =
					PROFILE_GENERATOR_INPUT_BASE_OFFSET +
					portIndex * PROFILE_GENERATOR_INPUT_SPACING;
				return {
					x: node.x,
					y: node.y + yOffset,
				};
		...
		if (type === 'in' && portId && isEditorialContentGeneratorNode(node)) {
			...
			if (portIndex !== -1) {
				// Calculate position matching MultiInputPort component layout
				const yOffset =
					EDITORIAL_INPUT_BASE_OFFSET + portIndex * EDITORIAL_INPUT_SPACING;
				return {
					x: node.x,
					y: node.y + yOffset,
				};
		...
		// Check if this is a multi-input node (comparison-data) with a specific port
		if (type === 'in' && portId && isComparisonDataNode(node)) {
			const portIndex = COMPARISON_DATA_PORT_ORDER.indexOf(portId);

			if (portIndex !== -1) {
				// Calculate position matching MultiInputPort component layout
				const yOffset =
					COMPARISON_DATA_INPUT_BASE_OFFSET +
					portIndex * COMPARISON_DATA_INPUT_SPACING;
				return {
					x: node.x,
					y: node.y + yOffset,
				};
		...
		// Check if this is a multi-input node (seo-optimization) with a specific port
		if (type === 'in' && portId && isSEOOptimizationNode(node)) {
			const portIndex = SEO_OPTIMIZATION_PORT_ORDER.indexOf(portId);
	...
	const renderTentativeConnection = (
		fromId: string,
	...
	return (
		...
		>
			...
			{connectingFrom &&
				renderTentativeConnection(connectingFrom, connectingFromPort)}
